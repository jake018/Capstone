...
Class Master extends DBConnection {
	...
	function place_order(){
		...
		while($vrow = $vendors->fetch_assoc()):
			...
			$payment_method = isset($_POST['payment_method']) ? $_POST['payment_method'] : ''; // Get the selected payment method
			
			$data = "('{$ref_code}','{$this->settings->userdata('id')}','{$vrow['id']}','{$this->conn->real_escape_string($delivery_address)}','{$payment_method}')";
			$sql = "INSERT INTO `order_list` (`code`,`client_id`,`vendor_id`,`delivery_address`, `payment_method`) VALUES {$data}";
			$save = $this->conn->query($sql);
